...
- **aitool/**: resolves the effective AI command. Priority is always: `WT_AI_TOOL` env (empty string = disabled) > config. Three variants: launch, resume (preset-specific flags like `--continue`), merge (for `sync --ai` / `merge --ai` / `done --ai`).
